...
std::optional<content::StoragePartitionConfig>
ContainedTabHandlerRegistry::MaybeInheritStoragePartition(
    const std::optional<content::StoragePartitionConfig>& storage_partition_key,
    content::SiteInstance* site_instance) {
  const content::StoragePartitionConfig* storage_partition_key_ptr = nullptr;
  if (storage_partition_key) {
    storage_partition_key_ptr = &storage_partition_key.value();
  } else if (site_instance) {
    storage_partition_key_ptr = &site_instance->GetStoragePartitionConfig();
  }

  if (!storage_partition_key_ptr) {
    return std::nullopt;
  }

  for (const auto& handler : handlers_) {
    if (handler->GetId() == storage_partition_key_ptr->partition_domain()) {
      return *storage_partition_key_ptr;
    }
  }
...
std::optional<GURL> ContainedTabHandlerRegistry::EmbedStoragePartitionKeyInUrl(
    const GURL& url,
  ...
  for (const auto& handler : handlers_) {
    if (handler->GetId() == storage_partition_key.first) {
      CHECK(!base::Contains(storage_partition_key.first,
                            kStoragePartitionKeySeparator));
      CHECK(!base::Contains(storage_partition_key.first, kSchemeSeparator));
      CHECK(!base::Contains(storage_partition_key.second,
                            kStoragePartitionKeySeparator));
      CHECK(!base::Contains(storage_partition_key.second, kSchemeSeparator));
      url_prefix = base::StrCat({
          storage_partition_key.first,
          kStoragePartitionKeySeparator,
          storage_partition_key.second,
          kSchemeSeparator,
      });
      return GURL(base::StrCat({url_prefix, url.spec()}));
    }
  }
...
ContainedTabHandlerRegistry::RestoreStoragePartitionKeyFromUrl(
  ...
  const auto parts =
      base::SplitStringPiece(url.scheme_piece(), kStoragePartitionKeySeparator,
    ...
    for (const auto& handler : handlers_) {
      if (parts[0] == handler->GetId()) {
        ...
        url_prefix_length = parts[0].length() +
                            ...
                            parts[1].length() + kSchemeSeparator.length();
...
// ContainedTabHandlerRegistry::GetPageStateTopUrlPrefix(const GURL& url) {
//   if (!url.is_valid()) {
//     return std::nullopt;
//   }

//   for (const auto& handler : handlers_) {
//     if (handler->GetId() == url.scheme_piece()) {
