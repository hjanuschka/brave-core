...
std::string_view TodoTool::Description() const {
  return "Creates, updates, or manages a structured task list for tracking "
         ...
         "multi-step "
         "tasks, demonstrate thoroughness, and provide visibility into "
         "task progress. Do not use this tool if the task is not complex "
         "enough to break it down in to discrete steps with nice titles. "
         "The "
         "tool handles creating new todo lists, "
         "updating existing todos, and managing task status through pending, "
         "in_progress, completed, and cancelled states. The task list is reset "
         "on every new human message. Do not use this tool if the tasks are "
         "not expected to take more than a few seconds or there are fewer than "
         "5 tasks. The user will see each other too's actions anyway so it "
         "will be duplicated if each task only equates to a single tool use by "
         "the assistant.";
}
...
void TodoTool::UseTool(const std::string& input_json,
  ...
  data_decoder::DataDecoder::ParseJsonIsolated(
      input_json,
      base::BindOnce(&TodoTool::OnJsonParsed, weak_ptr_factory_.GetWeakPtr(),
                     std::move(callback)));
}

void TodoTool::OnJsonParsed(UseToolCallback callback,
  ...
  if (!result.has_value()) {
    ...
    error_result.push_back(
        mojom::ContentBlock::NewTextContentBlock(mojom::TextContentBlock::New(
            "Error: Invalid JSON input - " + result.error())));
    std::move(callback).Run(std::move(error_result));
    return;
  }

  const base::Value::Dict* request_dict_param = result->GetIfDict();
  if (!request_dict_param) {
    ToolResult error_result;
    error_result.push_back(mojom::ContentBlock::NewTextContentBlock(
        mojom::TextContentBlock::New("Error: Input must be a JSON object")));
    std::move(callback).Run(std::move(error_result));
    return;
  }

  const base::Value::Dict& request_dict = *request_dict_param;
