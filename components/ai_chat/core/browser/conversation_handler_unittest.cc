...
TEST_F(ConversationHandlerUnitTest,
  ...
  EXPECT_CALL(*tool1_ptr, UseTool(StrEq("{\"location\":\"New York\"}"), _, _))
...
TEST_F(ConversationHandlerUnitTest, ToolUseEvents_MultipleToolsCalled) {
  ...
  EXPECT_CALL(*tool1_ptr, UseTool(StrEq("{\"location\":\"NYC\"}"), _, _))
      .InSequence(seq)
      .WillOnce(testing::WithArg<1>([&](Tool::UseToolCallback callback) {
        std::vector<mojom::ContentBlockPtr> result;
        result.push_back(mojom::ContentBlock::NewTextContentBlock(
  ...
  EXPECT_CALL(*tool2_ptr, UseTool(StrEq("{\"input1\":\"val1\"}"), _, _))
      .InSequence(seq)
      .WillOnce(testing::WithArg<1>([&](Tool::UseToolCallback callback) {
        std::vector<mojom::ContentBlockPtr> result;
        result.push_back(mojom::ContentBlock::NewTextContentBlock(
...
TEST_F(ConversationHandlerUnitTest, ToolUseEvents_MultipleToolIterations) {
  ...
  EXPECT_CALL(*tool1_ptr, UseTool(StrEq("{\"param1\":\"value1\"}"), _, _))
      .WillOnce(testing::WithArg<1>([&](Tool::UseToolCallback callback) {
        std::vector<mojom::ContentBlockPtr> result;
        result.push_back(mojom::ContentBlock::NewTextContentBlock(
            mojom::TextContentBlock::New("Result from tool1")));
  ...
  EXPECT_CALL(*tool2_ptr, UseTool(StrEq("{\"param2\":\"value2\"}"), _, _))
      .WillOnce(testing::WithArg<1>([&](Tool::UseToolCallback callback) {
        std::vector<mojom::ContentBlockPtr> result;
        result.push_back(mojom::ContentBlock::NewTextContentBlock(
            mojom::TextContentBlock::New("Result from tool2")));
...
TEST_F(ConversationHandlerUnitTest,
  ...
  std::vector<std::unique_ptr<Tool>> tools;
  tools.push_back(std::move(interactive_tool));
  tools.push_back(std::move(auto_tool));
  ON_CALL(*mock_tool_provider_, GetTools()).WillByDefault([&]() {
    std::vector<Tool*> tools;
    tools.push_back(interactive_tool_ptr);
    tools.push_back(auto_tool_ptr);
    return tools;
  });
